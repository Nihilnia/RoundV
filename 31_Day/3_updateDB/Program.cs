...
namespace _3_updateDB
{
    class Program
    {
        ...
        static void UpdateSongByID(int ID)
        {
            using (var DB = new SongsDB_02Context())
            {
                var theSong = new Song() { ID = 5 };
                DB.Songs.Attach(theSong);

                theSong.Artist = "Artist_055";
                ...
                //using (var DB = new SongsDB_02Context())
                //{
                //    var gonnaUpdate = DB.Songs.Where(s => s.ID == ID).FirstOrDefault();

                //    if(gonnaUpdate != null)
                //    {
                //        gonnaUpdate.Artist = "Gloria";
