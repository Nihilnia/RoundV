...
namespace _4_updateIntel
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Wanna update a customer?: ");
            string consoleUpdate = Console.ReadLine();
            if(consoleUpdate.ToLower() == "y")
            {
                Console.WriteLine("Give me the ID for the Update: ");
                string updateID = Console.ReadLine();
                SQL_Customers foundCustomer = sqlThingz.GetCustomerByID(updateID);
                ...
                sqlThingz.UpdateCustomer(updateID, newName, newCompany);
            }
        ...
        public class SqlProcess : ISQLThingz
        {
            ...
            public void UpdateCustomer(string ID, string Name, string Company)
            {
                using (var sConnection = GetSQLConnection())
                {
                    try
                    {
                        sConnection.Open();
                        ...
                        string sqlQuery = @"UPDATE Customers
                                            SET ContactName = @cName, CompanyName = @cCompany
                                            WHERE CustomerID = @cID";

                        SqlCommand command = new SqlCommand(sqlQuery, sConnection);
                        ...
                        int resultsX = command.ExecuteNonQuery();

                        Console.WriteLine($"{resultsX} rows updated..");
                    }
                    catch (Exception err)
                    {
                        Console.WriteLine("Somethings went wrong with SQL..");
