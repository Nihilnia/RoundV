...
namespace _3_updateDB
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (key == 1)
            {
                ...
                switch (userChoice)
                {
                    ...
                    case "11":
                        Console.WriteLine("How many witchez do you wanna add?: ");
                        int userWitch = Convert.ToInt32(Console.ReadLine());
                        AddToWitches(userWitch);
                        break;
        ...
        static void addToGloria()
        {
            using (var theDB = new SongsDB_02Context())
            {
                var theInfo = new Gloria() { Name = "Inf_01" };
                theDB.Glorian.Add(theInfo);

                theDB.SaveChanges();
            }
        }

        static void AddToWitches(int piece)
        {
            using (var daDB = new SongsDB_02Context())
            {
                List<Witch> daWitchez = new List<Witch>();

                for (int f = 0; f < piece; f++)
                {
                    daWitchez.Add(new Witch { Name = "Witch_" + piece });
                }

                daDB.Witches.AddRange(daWitchez);

                daDB.SaveChanges();

                Console.WriteLine($@"{piece} piece added to Witches table..");
            }
        }
    }
}